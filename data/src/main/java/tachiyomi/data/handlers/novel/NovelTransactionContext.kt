...
internal suspend fun AndroidNovelDatabaseHandler.getCurrentNovelDatabaseContext(): CoroutineContext {
    return coroutineContext[NovelTransactionElement]?.transactionDispatcher ?: queryDispatcher
}

internal suspend fun <T> AndroidNovelDatabaseHandler.withNovelTransaction(block: suspend () -> T): T {
    val transactionContext =
        coroutineContext[NovelTransactionElement]?.transactionDispatcher ?: createTransactionContext()
    return withContext(transactionContext) {
        val transactionElement = coroutineContext[NovelTransactionElement]!!
...
private suspend fun AndroidNovelDatabaseHandler.createTransactionContext(): CoroutineContext {
    ...
    val transactionElement = NovelTransactionElement(controlJob, dispatcher)
...
private class NovelTransactionElement(
    private val transactionThreadControlJob: Job,
    val transactionDispatcher: ContinuationInterceptor,
) : CoroutineContext.Element {

    companion object Key : CoroutineContext.Key<NovelTransactionElement>

    override val key: CoroutineContext.Key<NovelTransactionElement>
        get() = NovelTransactionElement
